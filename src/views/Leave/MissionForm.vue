		  ...
		  <div 
			class="overflow-hidden p-4 mb-4 bg-white transition-all duration-200 ease-in-out transform scale-100 grid grid-cols-1 gap-4 shadow-inner border border-gray-200">
				...
				<div class="grid grid-cols-1 md:grid-cols-3 gap-4 mt-5">
					<!-- Desinationlocation -->
					<div>
						<label for="leave-desinationlocation">Desination location / <span
								class="battambang-regular pt-2 text-[16px]">គោលដៅត្រូវទៅ</span><span
								class="text-red-800">**</span></label>
						<n-input v-model:value="form.desinationlocation" type="text" placeholder=""
							class="mt-3 font-bold text-black" />
					</div>
					...
					<!-- Reason for Leave -->
					<div>
						<label for="reason-for-leave" class="pt-2 text-[16px] ">Purpose of Requested / <span
								class="battambang-regular text-[16px]">គោលបំណងនៃការស្នើសុំ</span><span
								class="text-red-800">**</span></label>
...
const submitForm = async () => {
	...
	const confirmMessage = `
		...
		<p class="p-custom"><span class="custom-width">គោលដៅត្រូវទៅ(Desination Location)</span><span class="custom-span">${form.value.desinationlocation}</span></p>
