    ...
    <form @submit.prevent="submitForm">
      ...
      <div class="p-4 md:p-6 lg:p-10">
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <!-- Approver Email -->
          <!-- <div>
            <label for="approveremail" class="pt-2 text-[16px]">Approver Email / <span
                class="battambang-regular text-[16px]">អ៊ីមែលអ្នកអនុម័ត</span></label>
            <n-input v-model:value="approveremail.value" type="text" placeholder="" readonly
...
const submitForm = async () => {
  try {
    ...
    const payload = {
      ...
      emailapproverl1: form.value.emailapproverl1,
      emailapproverl2: form.value.emailapproverl2,
      emailapproverl3: form.value.emailapproverl3,
      approveremail: approveremail.value, // Use computed approveremail
      acknowledgeby: form.value.acknowledgeby,
    ...
    if (response.data.status === "200") {
      Swal.fire("Success", "ការបង្កើតទទួលបានជោគជ័យ!", "success").then(() => {
        router.push({ name: 'employee' });

      });
    } else {
      Swal.fire("Error", "ការបង្កើតរបស់អ្នកបរាជ័យ", "error");
    }
  } catch (error) {
    console.error("Error submitting request:", error);
    Swal.fire("Error", "មានបញ្ហាក្នុងការបង្កើត", "error");
  } finally {
